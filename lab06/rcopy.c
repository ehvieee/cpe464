...
void talkToServer(int socketNum, struct sockaddr_in6 * server)
{
	...
	uint8_t pduBuffer[1407];
	uint8_t recvBuffer[1047];
	int pduLen = 0;
	...
	while (buffer[0] != '.')
	{
		...
		safeRecvfrom(socketNum, recvBuffer, pduLen, 0, (struct sockaddr *) server, &serverAddrLen);
		
		// print out bytes received
		ipString = ipAddressToString(server);
		printf("Server with ip: %s and port %d:\n", ipString, ntohs(server->sin6_port));
		printPDU(recvBuffer, pduLen);
